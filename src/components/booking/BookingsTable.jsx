...
import { formatDateArray, convertToISO } from "../utils/Share"
	...
	const filterBooknigs = (startDate, endDate) => {
		...
		if (startDate && endDate) {
			filtered = bookingInfo.filter((booking) => {
				let checkInDate = convertToISO(booking.checkInDate)
				let checkOutDate = convertToISO(booking.checkOutDate)
				const bookingStarDate = parseISO(checkInDate)
				...
				<tbody className="text-center">
					{filteredBookings.map((booking, index) => (
						<tr key={booking.id}>
							...
							<td>{formatDateArray(booking.checkInDate)}</td>
							<td>{formatDateArray(booking.checkOutDate)}</td>
